...
$message1='';
$user=$_GET['use'];
$sql="SELECT first_name, last_name, staff_id, postal_address,phone,email,username, password from pharmacist WHERE username='$user'";
...
$result = mysqli_query($conn,$sql)or die(mysql_error());
if($result) {header("location:http://".$_SERVER['HTTP_HOST'].dirname($_SERVER['PHP_SELF'])."/admin.php");
}else{
		...
		<?php echo $message1;?>
          <form name="myform" onsubmit="return validateForm(this);" action="update_pharmacist.php" method="post" >
			<table width="420" height="106" border="0" >	
				...
				<tr><td align="center"><input name="username" type="text" style="width:170px" placeholder="Username" id="username" value="<?php include_once('connect_db.php'); echo $user?>" /></td></tr>
				<tr><td align="center"><input name="password" placeholder="Password" id="password" value="<?php include_once('connect_db.php'); echo $pas?>" type="password" style="width:170px"/></td></tr>
